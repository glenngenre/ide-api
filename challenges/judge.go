...
func BuildCaseResults(
	...
	for i := 0; i < len(expectedOutputs); i++ {
		...
		if harnessResult == nil {
			...
			passed := false
			result["passed"] = passed
			result["error"] = firstNonEmpty(harnessResult.Message, harnessResult.Error, harnessResult.Trace)
			if !hidden {
				result["stdout"] = ""
			}
		} else {
			passed := Equal(expectedOutputs[i], harnessResult.Value)
			...
			if !hidden {
				result["input"] = unmarshalInputArray(harnessResult.Value)
				result["expected"] = json.RawMessage(expectedOutputs[i])
				result["actual"] = harnessResult.Value
				result["stdout"] = ""
			}
		}
